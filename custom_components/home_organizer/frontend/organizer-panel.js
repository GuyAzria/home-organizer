...
class HomeOrganizerPanel extends HTMLElement {
      ...
      <div class="app-container" id="app">
        ...
        <div class="bottom-bar" id="add-area">
             <div style="display:flex;gap:10px;margin-bottom:10px">
                <button class="action-btn" id="btn-open-cam" style="background:#333;width:45px;height:45px;padding:0;display:flex;align-items:center;justify-content:center">${ICONS.camera}</button>
  ...
  bindEvents() {
    ...
    click('btn-ai-magic', () => { if (!this.tempAddImage) return alert("Take a picture first!"); this.callHA('ai_action', { mode: 'identify', image_data: this.tempAddImage }); });
  ...
  updateUI() {
    ...
    if (attrs.depth < 2) {
        ...
        const listContainer = document.createElement('div'); list.className = 'item-list';
  ...
  async handleDropAction(targetSubloc, itemName) {
      ...
      try { await this.callHA('clipboard_action', {action: 'cut', item_name: itemName}); await this.callHA('paste_item', {target_path: targetPath}); } catch (err) { console.error("Drop failed:", err); }
  }
  ...
  createItemRow(item, isShopMode) {
     ...
     div.innerHTML = `<div class="item-main" onclick="this.getRootNode().host.toggleRow('${item.name}')"><div class="item-left">${iconHtml}<div><div>${item.name}</div><div class="sub-title">${subText}</div></div></div><div class="item-qty-ctrl">${controls}</div></div>`;
