// Home Organizer Ultimate - Ver 7.1.3 (Two-Step AI UI + Invoice Scan)
// License: MIT
...
class HomeOrganizerPanel extends HTMLElement {
  ...
  renderChatUI(container) {
      ...
      // BUILD INPUT BAR WITH TEMPLATE TO ENSURE ICON DISPLAY
  ...
  compressImage(file, callback, applyBgFilter = false) { const reader = new FileReader(); reader.onload = (e) => { const img = new Image(); img.onload = () => { const canvas = document.createElement('canvas'); const ctx = canvas.getContext('2d'); const MAX = 1024; let w = img.width, h = img.height; if (w > h) { if (w > MAX) { h *= MAX/w; w = MAX; } } else { if (h > MAX) { w *= MAX/h; h = MAX; } } canvas.width = w; canvas.height = h; ctx.drawImage(img, 0, 0, w, h); if (applyBgFilter) { let imageData = ctx.getImageData(0, 0, canvas.width, canvas.height); let data = imageData.data; for (let i = 0; i < data.length; i += 4) { let r = data[i], g = data[i+1], b = data[i+2]; if (r > 190 && g > 190 && b > 190) { data[i] = 255; data[i+1] = 255; data[i+2] = 255; } } ctx.putImageData(imageData, 0, 0); } callback(canvas.toDataURL('image/jpeg', 0.5)); }; img.src = e.target.result; }; reader.readAsDataURL(file); }
