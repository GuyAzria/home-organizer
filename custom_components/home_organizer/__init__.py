...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Register Panel - Indentation Fixed
...
async def register_services(hass, entry):
    ...
    async def handle_navigate(call):
        ...
        if direction == "root": hass.data[DOMAIN]["current_path"] = []
        elif direction == "up":
            if current_path: current_path.pop()
        elif direction == "down":
            if name: current_path.append(name)
    ...
    async def handle_ai_action(call):
        ...
        payload = { "contents": [{ "parts": [{ "text": prompt_text }, { "inlineData": { "mimeType": "image/jpeg", "data": img_b64 } }] }] }
        try:
            ...
            async with session.post(url, json=payload) as resp:
                if resp.status == 200:
                    ...
                    try:
                        ...
                        if mode == 'identify': hass.data[DOMAIN]["ai_suggestion"] = text
                        elif mode == 'search':
                            hass.data[DOMAIN]["search_query"] = text
                            hass.data[DOMAIN]["shopping_mode"] = False
                        await update_wrapper(hass)
                    except Exception as e: _LOGGER.error(f"Failed to parse AI response: {e}")
                else: _LOGGER.error(f"Gemini API Error {resp.status}")
        except Exception as e: _LOGGER.error(f"AI Connection Error: {e}")
